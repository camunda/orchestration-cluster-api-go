...
type ApiGetClusterUpgradeStatusRequest struct {
	ctx        context.Context
	ApiService *ClusterAPIService
}

func (r ApiGetClusterUpgradeStatusRequest) Execute() (*ClusterUpgradeStatusResponse, *http.Response, error) {
	return r.ApiService.GetClusterUpgradeStatusExecute(r)
}

/*
GetClusterUpgradeStatus Get the upgrade-readiness status of the whole cluster
...
Reports one overall upgrade-readiness status for the whole cluster, folded over every physical tenant and condition. `MIGRATED` only once every known condition has migrated for every known physical tenant; `MIGRATION_IN_PROGRESS` when at least one is confirmed not yet migrated; `UNKNOWN` otherwise (including before anything has been reported yet). No per-tenant or per-condition detail is reported here; see the `upgradeReadiness` actuator endpoint for that.
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiGetClusterUpgradeStatusRequest
*/
func (a *ClusterAPIService) GetClusterUpgradeStatus(ctx context.Context) ApiGetClusterUpgradeStatusRequest {
	return ApiGetClusterUpgradeStatusRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
// Execute executes the request
//
//	@return ClusterUpgradeStatusResponse
func (a *ClusterAPIService) GetClusterUpgradeStatusExecute(r ApiGetClusterUpgradeStatusRequest) (*ClusterUpgradeStatusResponse, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *ClusterUpgradeStatusResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ClusterAPIService.GetClusterUpgradeStatus")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/cluster/v2/status/upgrade"
